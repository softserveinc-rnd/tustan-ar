...
namespace TustanAR.Smoothing{
	...
	/// Interface to access model smoothing.
    public interface ISmoothManager{

			/// Called when Vuforia target is found.
			void OnTargetFound(GameObject castle, GameObject imageTarget);
			
			/// Called when Vuforia Target is lost.
			void OnTargetLost();

			/// Called on every frame. All smoothing is here.
			void OnTargetUpdate();
		}
}